...
<body>
  ...
  <table width="100%" border="1">
    ...
    <tr>
      ...
      <?php if (($_GET['jnsmesin'] != "lsr") && ($_GET['mesin'] != "P3LS101")): ?>
        <td rowspan="3">
          <div align="center"><font size="-2">VMT</font></div>
        </td>
        <td rowspan="3">
          <div align="center"><font size="-2">OVER FEED</font></div>
        </td>
        <td rowspan="3">
          <div align="center"><font size="-2">BUKA RANTAI</font></div>
        </td>
      <?php endif; ?>
      <td colspan="2">
        <div align="center">
          ...
          <td>
            <font size="-2">
              ...
              if ($rowd['vmt']) {
                ...
                $q_QA_DATA = db2_exec($conn_db2, "SELECT VALUEQUANTITY FROM ITXVIEW_DETAIL_QA_DATA
                  WHERE PRODUCTIONORDERCODE = '$rowd[nokk]' 
                    AND PRODUCTIONDEMANDCODE = '$rowd[demandno]'
                    AND OPERATIONCODE = '$rowd[nama_mesin]'
                    AND CHARACTERISTICCODE = 'VMT'
                  ORDER BY LINE ASC");
                $row_QA_DATA = db2_fetch_assoc($q_QA_DATA);
                echo round($row_QA_DATA['VALUEQUANTITY'], 2);
              }
