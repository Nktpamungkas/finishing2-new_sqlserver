...
<body>
  ...
  if (isset($_POST['btnHapus'])) {
    $hapusSql = "DELETE FROM db_finishing.tbl_energi WHERE id='$_POST[id]'";
  ...
  if (isset($_POST['btnSimpan'])) {
    ...
    $simpanSql = "INSERT INTO db_finishing.tbl_energi (kode,ket) VALUES ('$kode','$ket')";
  ...
  if (isset($_POST['btnUbah'])) {
    $kode = $_POST['kode'];
    $ket = str_replace("'", "", $_POST['ket']);
    $simpanSql = "UPDATE db_finishing.tbl_energi SET 
    kode='$kode',
    ket='$ket'
    WHERE id='$_POST[id]'";
    sqlsrv_query($con, $simpanSql);
  ...
  <form id="form1" name="form1" method="post" action="" enctype="multipart/form-data">
    <table width="100%" border="0">
      ...
      <?php $qtampil = sqlsrv_query($con, "SELECT TOP 1 * FROM db_finishing.tbl_energi WHERE kode='$_GET[kode]'", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
    ...
    <table width="100%" border="0">
      ...
      <?php
      $qry = sqlsrv_query($con, "SELECT * FROM db_finishing.tbl_energi ORDER BY id ASC");
      $no = 1;
