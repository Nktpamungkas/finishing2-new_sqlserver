...
<body>
  ...
  function nourut()
  {
    include('../koneksi.php');
    $format = date("ymd");
    $sql = sqlsrv_query($con, "SELECT TOP 1 nokk FROM db_finishing.tbl_adm WHERE SUBSTRING(nokk,1,6) LIKE '%" . $format . "%' ORDER BY nokk DESC ", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
  ...
  if ($idkk != "") {
    ...
    $qry1 = sqlsrv_query($con, "SELECT TOP 1 * FROM db_finishing.tbl_adm WHERE nokk='$idkk' AND status='2'  ORDER BY id DESC ", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
    $rw1 = sqlsrv_fetch_array($qry1);
    $rc1 = sqlsrv_num_rows($qry1);

    $qry = sqlsrv_query($con, "SELECT TOP 1 * FROM db_finishing.tbl_adm WHERE nokk='$idkk' AND status='1' AND tgl_out IS NULL ORDER BY id DESC ", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
  ...
  if (isset($_POST['btnSimpan'])) {
    ...
    $simpanSql = "UPDATE db_finishing.tbl_adm SET
    shift_out='$shift',
    shift1_out='$shift1',
    catatan='$note',
    tujuan='$tujuan',
      ...
      <table width="100%" border="0">
        ...
        <tr>
          ...
          <td><select name="tujuan" id="tujuan" required="required">
              <option value="">Pilih</option>
              <?php $qry1 = sqlsrv_query($con, "SELECT tujuan FROM db_finishing.tbl_tujuan ORDER BY id ASC");
