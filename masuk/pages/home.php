...
<body>
	...
	function nourut()
	{
		...
		$format = date("ymd");
		// $format = '231213';
		$sql = sqlsrv_query($con, "SELECT TOP 1 nokk FROM db_finishing.tbl_produksi WHERE SUBSTRING(nokk,1,6) like '%" . $format . "%' ORDER BY nokk DESC", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
	...
	if ($_GET['typekk'] == "KKLama") {
		echo "<script>
						swal({
							...
							if (result.value) {
								window.location.href = 'http://10.0.0.10/finishing2-new_sqlserver/masuk/?typekk=NOW'; 
							}
						});
					</script>";
		...
		if ($idkk != null) {
			...
			$q_schedule = sqlsrv_query($con, "SELECT * FROM db_finishing.tbl_schedule_new WHERE nokk = '$idkk' AND nodemand = '$_GET[demand]' AND operation = '$_GET[operation]'");
			$row_schedule = sqlsrv_fetch_array($q_schedule, SQLSRV_FETCH_ASSOC);

			$q_kkmasuk = sqlsrv_query($con, "SELECT * FROM db_finishing.tbl_masuk WHERE nokk = '$idkk' AND nodemand = '$_GET[demand]' AND operation = '$_GET[operation]'");
	...
	if (isset($_POST['btnSimpan'])) {
		...
		if ($row_kkproses) {
			...
			$nokk = $_POST['nokk'];
			$demand = $_POST['demand'];
			sqlsrv_query($con, "INSERT INTO db_finishing.tbl_log(akun, ipaddress, creationdatetime, catatan, [status]) VALUES('$_SESSION[usr]', '$_SERVER[REMOTE_ADDR]', '$creationdatetime', 'Aktivitas Illegal, Schedule input double. $nokk, $demand', 'kkmasuk')");
			echo "<script>
								swal({
