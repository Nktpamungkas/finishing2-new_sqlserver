...
<table>
    ...
    <tbody>
        ...
        $q_tblmasuk = sqlsrv_query($con, "SELECT * FROM db_finishing.tbl_masuk WHERE [status] = 'KK MASUK' $where_tgl $where_nama_mesin");
        ?>
        <?php while ($row_tblmasuk = sqlsrv_fetch_array($q_tblmasuk)): ?>
            ...
            $cek_schedule = sqlsrv_query($con, "SELECT COUNT(*) AS jml FROM db_finishing.tbl_schedule_new WHERE nokk = '$row_tblmasuk[nokk]' AND nodemand = '$row_tblmasuk[nodemand]'");
            $data_schedule = sqlsrv_fetch_array($cek_schedule, SQLSRV_FETCH_ASSOC);
            ?>
            <?php if (empty($data_schedule['jml'])): ?>
                <tr>
                    ...
                    <td><?php if ($row_tblmasuk['tgl_delivery'] != null or $row_tblmasuk['tgl_delivery'] != '') {
                        echo $row_tblmasuk['tgl_delivery']->format('Y-m-d');
                    } else {
                        echo '';
                    } ?></td>
                    ...
                    <td><?php if ($row_tblmasuk['creationdatetime'] != null or $row_tblmasuk['creationdatetime'] != '') {
