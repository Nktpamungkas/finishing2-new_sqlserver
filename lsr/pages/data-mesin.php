...
<body>
  ...
  if (isset($_POST['btnSimpan'])) {
    ...
    $params = array($no_mesin, $_POST['jenis'], $ket);
    $stmt = sqlsrv_query($con, $simpanSql, $params);

    if (!$stmt) {
      die("Gagal Simpan: " . var_dump(sqlsrv_errors()));
    }
      ...
      <tr>
        ...
        <td width="78%"><label for="no_mesin"></label>
          <input type="text" name="no_mesin" id="no_mesin"
            onchange="window.location='data-mesin.php?no_mesin='+this.value" value="<?php echo $_GET['no_mesin']; ?>"
            required="required" />
