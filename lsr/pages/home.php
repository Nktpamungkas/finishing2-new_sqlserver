...
<body>
    ...
    function cek($value)
    {
        ...
        if ($_GET['typekk'] == "KKLama") {
            ...
            //                 }).then((result) => {
            //                     if (result.value) {
            //                         window.location.href = 'http://online.indotaichen.com/finishing2-new/lsr/?typekk=SCHEDULE'; 
            //                     }
            //                 });
            //             </script>";
...
$proses_menit,
    ...
    <form id="form1" name="form1" method="post" action="">
        <fieldset>
            <legend>Data Produksi Harian LSR</legend>
            <table width="100%" border="0">
                            ...
                            $qry1 = db2_exec($conn_db2, "SELECT
                                                            *
                                                            FROM
                                                                RESOURCES r
                                                            WHERE
                                                                SUBSTR(CODE, 1,4) = 'P3LS' $where_schedule
                                                            ORDER BY 
                                                                SUBSTR(CODE, 6,2) 
                                                            ASC");
                ...
                <tr>
                    ...
                    <td colspan="2"><select name="proses" id="proses" required>
                            <option value="">Pilih</option>
                            <?php
                            $qry1 = sqlsrv_query($con, "SELECT proses, jns FROM db_finishing.tbl_proses WHERE ket='lsr' ORDER BY proses ASC");
                            while ($r = sqlsrv_fetch_array($qry1)) {
                            ?>
