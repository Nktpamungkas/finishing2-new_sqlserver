		...
		<table width="650" border="0">
			...
			<tr>
				...
				<td>
					<select name="kksudahproses" class="form-control select2" required>
						...
						<option value="1" <?php if($_POST['kksudahproses'] == '1'){ echo "SELECTED"; } ?>>Tampilkan
							Semua</option>
						<option value="2" <?php if($_POST['kksudahproses'] == '2'){ echo "SELECTED"; } ?>>Hanya
							tampilkan data sudah proses</option>
						<option value="3" <?php if($_POST['kksudahproses'] == '3'){ echo "SELECTED"; } ?>>Hanya
							tampilkan data yang belum proses</option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="nourut" class="form-control select2" required>
						<option value="" disabled selected>Pilih</option>
						<option value="with0" <?php if($_POST['nourut'] == 'with0'){ echo "SELECTED"; } ?>>Semua nomor
							urut dengan 0</option>
						<option value="without0" <?php if($_POST['nourut'] == 'without0'){ echo "SELECTED"; } ?>>Semua
							nomor urut tidak dengan 0</option>
						<?php
                            $q_nourut    = sqlsrv_query($con, "SELECT
                                                                ...
                                                                ORDER BY
                                                                    nourut ASC");
                        ?>
						<?php while ($row_nourut = sqlsrv_fetch_array($q_nourut,SQLSRV_FETCH_ASSOC)) : ?>
					...
					<select name="no_mesin" class="form-control select2">
                        ...
                        $q_mesin    = sqlsrv_query($con, "SELECT DISTINCT
                                                        ...
                                                        FROM
                                                            db_finishing.tbl_schedule_new
                                                        ORDER BY
                                                            SUBSTRING(TRIM(no_mesin), LEN(TRIM(no_mesin)) - 4, 2) ASC,
                                                            SUBSTRING(TRIM(no_mesin), LEN(TRIM(no_mesin)) - 1, 2) ASC;");
                        ?>
						...
						<?php
                            $q_proses    = sqlsrv_query($con, "SELECT
                                                            ...
                                                            FROM
                                                                  db_finishing.tbl_schedule_new");
                        ?>
						<?php while ($row_proses = sqlsrv_fetch_array($q_proses,SQLSRV_FETCH_ASSOC)) : ?>
			...
			<tr valign="middle">
				...
				<td width="280"><input name="awal" type="text" id="awal" value="<?= $_POST['awal'] ?>"
						onclick="if(self.gfPop)gfPop.fPopCalendar(document.form1.awal);return false;" size="14" /><a
			...
			<tr>
				...
				<td width="280"><input name="akhir" type="text" id="akhir" value="<?= $_POST['akhir'] ?>"
						onclick="if(self.gfPop)gfPop.fPopCalendar(document.form1.akhir);return false;" size="14" /><a
						href="javascript:void(0)"
			...
			<tr>
				<td colspan="3">
					...
					<input type="button" name="batal" value="Reset" onclick="window.location.href='index.php?p=Reports'"
						class="art-button">
					<input type="button" name="batal" value="Kembali"
		...
		<tbody>
			...
			<tr>
				...
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['no_mesin']; ?> <br>
					<?= substr(TRIM($row_schedule['no_mesin']), -5, 2).substr(TRIM($row_schedule['no_mesin']), -2); ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['nama_mesin'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['group_shift'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['nokk'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['nodemand'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['langganan'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['buyer'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['no_order'] ?>
				</td>
				<?php
                ...
                ?>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= htmlspecialchars($tgl_delivery) ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: left;"><?= $row_schedule['jenis_kain'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['lebar'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['gramasi'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: left;"><?= $row_schedule['warna'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['no_warna'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['lot'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['roll'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['qty_order'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['qty_order_yd'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['operation'] ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['proses'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;"><?= $row_schedule['personil'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: left;"><?= $row_schedule['catatan'] ?>
				</td>
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['creationdatetime'] ?></td>
			...
			<tr>
				...
				<td style="border:1px solid;vertical-align:middle; text-align: center;">
					<?= $row_schedule['creationdatetime']->format('Y-m-d') ?></td>
				<td style="border:1px solid;vertical-align:middle; text-align: left;">
					<?php
                                    // CEK JIKA SUDAH PROSES MAKA MUNCULIN DI KETERANGAN
                                    $cek_hasilproses   = sqlsrv_query($con, "SELECT * FROM  db_finishing.tbl_produksi WHERE nokk = '$row_schedule[nokk]' AND demandno = '$row_schedule[nodemand]' AND no_mesin = '$row_schedule[no_mesin]' AND nama_mesin = '$row_schedule[operation]'");
