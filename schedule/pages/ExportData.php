...
<table border="1">
	<thead>
		<tr>
			...
			<th colspan="23" align="center" valign="middle">
				<strong>
					<font size="+1">
						SCHEDULE FINISHING <?php if (empty($_GET['no_mesin'])) {
							echo "SEMUA MESIN";
						} ?>
					</font>
					<br>
	...
	<tbody>
		...
		if ($_GET['awal']) {
			$where_tgl = "AND CONVERT(DATE,creationdatetime)BETWEEN '$_GET[awal]' AND '$_GET[akhir]'";
		} else {
			$where_tgl = "";
		}
		$no = 1;
		$query_schedule = "SELECT *
							...
							ORDER BY
								CONCAT(
									SUBSTRING(LTRIM(no_mesin), LEN(LTRIM(no_mesin)) - 5, 2),
									SUBSTRING(LTRIM(no_mesin), LEN(LTRIM(no_mesin)) - 2, 2)
								) ASC,
								nourut ASC";
		...
		<?php while ($row_schedule = sqlsrv_fetch_array($q_schedule, SQLSRV_FETCH_ASSOC)): ?>
			...
			<?php if (empty($data_proses['jml']) and $_GET['kksudahproses'] == '3'): ?>
				<tr>
					...
					<td style="white-space: nowrap;"><a target="_BLANK"
			...
			<?php elseif (!empty($data_proses['jml']) and $_GET['kksudahproses'] == '2'): ?>
				<tr>
					...
					?>
					<td style="white-space: nowrap;"><?= $tgl_delivery ?></td>
					...
					<td style="white-space: nowrap;"><?= $row_schedule['personil'] ?></td>
					<td style="white-space: nowrap;"><?= $row_schedule['catatan'] ?></td>
					<td style="white-space: nowrap;"><?= $row_schedule['creationdatetime']->format('Y-m-d') ?></td>
					<td style="white-space: nowrap;">
						<?php
						// CEK JIKA SUDAH PROSES MAKA MUNCULIN DI KETERANGAN
						$cek_hasilproses = sqlsrv_query($con, "SELECT * FROM db_finishing.tbl_produksi WHERE nokk = '$row_schedule[nokk]' AND demandno = '$row_schedule[nodemand]' AND no_mesin = '$row_schedule[no_mesin]' AND nama_mesin = '$row_schedule[operation]'");
						...
						<?= $data_hasilproses['tgl_buat']->format('Y-m-d'); ?>,
						<?= $data_hasilproses['no_mesin']; ?>,
						<?= $data_hasilproses['nama_mesin']; ?>-<?= $data_hasilproses['proses']; ?>
					</td>
				</tr>
			<?php elseif ((!empty($data_proses['jml']) or empty($data_proses['jml'])) and $_GET['kksudahproses'] == '1'): ?>
				<tr>
					...
					if ($row_schedule['creationdatetime'] instanceof DateTime) {
						// Format objek DateTime ke 'Y-m-d'
						$creationdatetime = $row_schedule['creationdatetime']->format('Y-m-d');
					} else {
						// Jika tidak, atur $tgl_delivery menjadi string kosong
						$creationdatetime = ''; // Atau gunakan nilai default sesuai kebutuhan
					}
					?>
					<td style="white-space: nowrap;"><?= $creationdatetime ?></td>
					<td style="white-space: nowrap;">
						<?php
						// CEK JIKA SUDAH PROSES MAKA MUNCULIN DI KETERANGAN
						$cek_hasilproses = sqlsrv_query($con, "SELECT * FROM db_finishing.tbl_produksi WHERE nokk = '$row_schedule[nokk]' AND demandno = '$row_schedule[nodemand]' AND no_mesin = '$row_schedule[no_mesin]' AND nama_mesin = '$row_schedule[operation]'");
						...
						<?php
						// Periksa apakah $row_schedule['tgl_delivery'] adalah objek DateTime
						if ($row_schedule['tgl_buat'] instanceof DateTime) {
							// Format objek DateTime ke 'Y-m-d'
							$tgl_buat = $row_schedule['tgl_buat']->format('Y-m-d');
						} else {
							// Jika tidak, atur $tgl_delivery menjadi string kosong
							$tgl_buat = ''; // Atau gunakan nilai default sesuai kebutuhan
						}
						?>
						<?= $tgl_buat ?>,
						<?= $data_hasilproses['no_mesin']; ?>,
